...
from ClassifyFunc.RELIEFSVM_CV import RELIEFFSVM_kFold
...
# GreyMaskDir = '..\PD_HC_Smooth/01rgrey.nii'
...
for f in np.arange(0.009,0.013,0.001):
    fea = f*n_features
    retain_features = int(fea)
    ResultantFolder = os.path.join('Results/reliefF%d/'%(retain_features))
    print("**********The experiment of retaining %d fetures begins"%(retain_features))
    for i in range(nIter):
        acc,sen,spe= RELIEFFSVM_kFold(SubjectsData,SubjectsLabel,ResultantFolder,kFold,NormalizeFlag,NormalzeMode,i,retain_features)
        Accuracies[i, 0] = acc
