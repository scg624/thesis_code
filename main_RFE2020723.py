...
from ClassifyFunc.RFE_CV import RFESVM_kFold
...
for f in np.arange(0.006,0.04,0.001):
    fea = f*n_features
    retain_features = int(fea)
    ResultantFolder = os.path.join('Results/RFE_GM_Index%d/'%(retain_features))
    print("**********The experiment of retaining %d fetures begins"%(retain_features))
    for i in range(nIter):
        acc,sen,spe= RFESVM_kFold(SubjectsData,SubjectsLabel,ResultantFolder,kFold,NormalizeFlag,NormalzeMode,i,retain_features)
        Accuracies[i, 0] = acc
